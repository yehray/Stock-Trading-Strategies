...
def test_run():
    ...
    rolling_mean_short = get_rolling_mean(df['AAPL'], window=10)
    ...
    ax = df['AAPL'].plot(title="AAPL Moving Average Strategy", label='AAPL')
    rolling_mean_short.plot(label='Rolling mean', color='green', ax=ax)

    # Add lines
    ax.vlines(long_stop_df.index, color='red', ymin=0, ymax=300)
