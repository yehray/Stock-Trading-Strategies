...
def test_run():
    ...
    rolling_mean = get_rolling_mean(df['AAPL'], window=20)

    # 2. Compute rolling standard deviation
    rolling_std = get_rolling_std(df['AAPL'], window=20)
    ...
    symbol_short_stop = ['AAPL'] * len(short_stop)
    ...
    symbol_long_stop = ['AAPL'] * len(long_stop)
    ...
    ax = df['AAPL'].plot(title="AAPL Bollinger Bands Strategy", label='AAPL')
    rolling_mean.plot(label='Rolling mean', ax=ax)
    upper_band.plot(label='Upper Band', ax=ax)
    lower_band.plot(label='Lower Band', ax=ax)

    # Add lines
    ax.vlines(short_df.index, color='red', ymin=0, ymax=300)
    ax.vlines(short_stop_df.index, color='black', ymin=0, ymax=300)
    ax.vlines(long_df.index, color='green', ymin=0, ymax=300)
    ax.vlines(long_stop_df.index, color='black', ymin=0, ymax=300)
