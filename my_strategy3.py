...
def test_run():
    ...
    rolling_mean = get_rolling_mean(df['IBM'], window=10)

    df_shift = df.shift(periods=1)
    rolling_mean_shift = rolling_mean.shift(periods=1)


    frames = [df, df_shift, rolling_mean, rolling_mean_shift]
    result = pd.concat(frames, axis=1)
    result = result.ix[20:]
    result.columns = ['df', 'df_shift', 'rolling_mean', 'rolling_mean_shift']
    print(result)
    ...
    for result_index, result_row in result.iterrows():
        if result_row['df_shift'] <= result_row['rolling_mean_shift'] and result_row['df'] > result_row['rolling_mean'] and short_flag == 0 and long_flag == 0:
            short_flag = 1
            short_stop_flag = 1
    ...
    # Add lines
    ax.vlines(short_df.index, color='red', ymin=0, ymax=130)
    ax.vlines(short_stop_df.index, color='black', ymin=0, ymax=130)
    ax.vlines(long_df.index, color='green', ymin=0, ymax=130)
    ax.vlines(long_stop_df.index, color='black', ymin=0, ymax=130)
